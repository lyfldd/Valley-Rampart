...
> 状态：✅已裁决（2026-08-28 · 四点全裁+补裁三条，裁决正文见文末 §六）
...
## 六、策划裁决（2026-08-28 · 策划端）

**抽查记录**：策划端实读核实报告 11 项关键声明——TerritorySystem 只读/唯一写点 RebuildInitial、ResolveOwnerCampCell 恒-1（L86）、条件4 占位恒真（L67）、TryAnnex 管线已备、FoundFromCamp 插旗后无领土写入（缺口①）、UtilityActionConfig.actions 实数 14 条无⑩（缺口②）、Expand 枚举位在（UtilityScorer L29）、DayCycle 步骤3 预留位、EnterPlaying 无条件 RebuildInitial（L129-130）、实施计划 L208-216 数字照抄未发明、冒烟 #6/#15/#18 无对应文件（glob 证实）——**11/11 属实**。缺口①②诚实自曝，勘察质量高，**嘉奖**。

### 裁1（问a·⑩minStage）：准 minStage=Expand
备选"Develop 后期"在现行 minStage 四段可见性门控中不可表达（伪选项）；⑨BuildWall 同段先例。

### 裁2（问a·TerritoryGap 缺口公式）：否原案，裁 A′
- 原案 `clamp01(非初始占区数/needA)` **方向反=开局死锁**：0 块起步 score=0 → ScoreTop `need<=0.0001 出局`（UtilityScorer L49）→ ⑩永不入选 → 永不推进 → score 恒 0。
- 备选B（额度余量）同否：D327 额度是容量门不是欲望，开局 (4+5)/96≈0.09 永选不中。
- **A′**：`TerritoryGap = clamp01((needA − 非初始占区数) / needA)`，needA=6 占位 SO 化；与 NeedScore 现行 Gap 族（CapacityGap/RecruitWorkerGap/WallGap：不足则高、达标归零）同构。**欲望与容量分离**：D327 额度公式只留 ExpandTick 做硬容量门。

### 裁3（问b·吞并即时性）：裁 A 日 tick
实施计划 L214 事件联动字面**让位**（记方式偏差，序1 先例）。叠加利好：五步权威序中领土 tick=步骤3 先于 CampUpgrader=步骤4，圈入/推进**当日即被吞并判定看到**（延迟最迟 1 日、常见 0 日），优于报告估的"最迟次日"。条件4 同源真判定照落（双保险防"有主仍立国"）。

### 裁4（问d·玩家纳土边界）：确认只纳无主
D327 字面 + D283 防飞地；他国领地玩家建造=静默不动，无领土变更、不吞并。**补遗**（照 HH.19 口径不另请示）：ClaimInitial / ExpandTick / ClaimAdjacentUnclaimed 三处写入均广播 TerritoryChangedEvent（按坐标序保确定性），供 2_10 染色消费。

### 补裁三条
1. **DZ-008 到期必落**：队列 Q1 已将「Count=8 上限 staging 探针」归属步骤12 联测，报告 §四验收漏列——批A 联测必含两探针（8 国满员拦截立国 + 吞并路径不受上限影响）。
2. **④债三探针准**（读档保真/新游戏重推/旧档无段兜底），照 §一问c。领土段=独立 SaveId（"TerritorySystem"、Global phase），**勿夹带 kingdoms[] 2_11 迁移债**，批C 边界写清。
3. **三批制照 §三放行，批A 先行**（玩家零接触）；每批单 commit + P0 基线；玩家侧破基线即停手报裁。

> 状态回写：HH.32 ✅已裁决；队列 Q1 → 🚧（批A 放行）。执行端按本 §六动工，交付报告带锚点声明（vr-triage-flow §四）。
