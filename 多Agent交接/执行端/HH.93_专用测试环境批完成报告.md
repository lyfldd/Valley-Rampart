...
## 九、策划端验收裁决（D552 · 2026-09-07 · 策划端实盘复核）

> 裁决文书同步落 0.6 §八十二；账本 HH.92/HH.93 本串销号；教训库 L-11/L-12 入库；底表 E10 改「复归」结案；P0 清单件F 归口注入。

**结论：✅ 验收成立**——T0~T19 二十项全勾；MVP 四条按 §9.1② 合格线判定成立；本批质量突出点=实盘定性推翻策划预判（T0，策划端已用 grep 复核 BuildingComponents.cs:150-153 挂载链+RulerController.cs:315 轮询口径证实）+如实披露容器自身判定缺陷+THD v2 实测自驱三修，均嘉奖。

### 9.1 T0~T19 勾选裁决（全成立，三点注记采纳）

| 注记 | 裁决 |
|---|---|
| ①T0 推翻清单预判 | **采纳**——ThroneAnchor=活引用非退役死代码（BuildingComponents.cs:152 主城 AddComponent+Update 0.5s 轮询 IsKingdomLost 工人全灭[口径出处 D249]→GameOver=D111 真实停跑机制）；「禁臆断实测定性」纪律执行到位。底表 E10 已由策划端改「复归」结案（D552，两行修订+迁移标记）；T14 核对册 §二 建议行同步销案 |
| ②A2 扩军=0 | **验收合格线=测试环境已能稳定复现该缺口**（seed=907 三轮实锤，断言保留未降标准）——执行端不越界修 AI 纪律正确；缺口本体归 P0 调优批（§9.3/件F-T1）。「缺口从不可复现变为固定 seed 稳定复现的断言」正是本批价值声明，采信 |
| ③THD v3 汇总行「ALL PASS」误显 | **采信以断言明细为准**（A1/A3/A4/T15/T9 全绿，A4 异常 8/8/4586→0/0/0）；容器判定缺陷已修（Valley_TestHarnessDemo.cs L145-152）追认入库，断言本体零改动——「汇总行不可作验收口径，只认断言明细」已并入教训库 L-11 防范动作 |

### 9.2 T6 三档 Draft v2 数值：**核准 + 迁 SO 授权**

- 三档数值表（开局 6/0/3 房 · 中期 10/4/5 · 军事 12/6/8）**全档核准**：①房容>人口不变式三档全成立（9>6/15>14/24>18，House 容量实测 3 的修订依据采信）；②军事期战士 6+needA=8 留扩军缺口空间=与 A2 断言意图自洽。
- **军事期 gold=1500 维持，不过富**：day2 即耗至个位数不是 fixture 缺陷，**正是 P0 调优批要观测的现象本体**（建造行动评分吃金速率 vs ⑦招战士竞争=件A 门槛映射/假设树的直接诊断输入）；压低起步金反而提前饿死观察窗、遮蔽缺口。若 P0 诊断需消歧「无金可招 vs 评分不选」两假设：SO 化后调参做对照轮即可，零成本（授权预给，见件F-T3）。
- 授权执行端：TestFixtureApi.cs Draft 表删除，数值迁 SO 资产（清单 T6 原文「档位数值 SO 化」兑现）——本节即 §六 所述「核准后迁 SO」的核准书。

### 9.3 列报归口裁决（§五）

| 列报项 | 归口 | 优先级/条件 |
|---|---|---|
| A2 扩军=0 | P0 调优批（件F-T1，与件A 诊断域同源） | 高 |
| 寻路不可达→Idle 循环（npc 66/72） | P0 调优批（件F-T2）——**首日先做可达性归因定性**（fixture 注入点不可达 / AI 目标选择 / 寻路系统回归三选一）；定性=寻路系统回归则转工程缺陷单，不入调参 | 中 |
| gold 消耗偏快 | P0 调优批（件F-T3，与 T6 富裕度耦合；消歧对照轮授权在案） | 高 |
| 逐条行动强制触发冒烟（21 条行为级） | P0 调优批扩展件（件F-T4）——T16 差集（从未被评分自然选中者）=件A 评分参数直接输入（L-10 同款收益路径自查） | 中 |
| 1x 档对照 | **条件触发**：15x 出现不可归因假红时先跑 10x，1x 仅终局抽验（结构等价论证 §四 T4 行采信；wall-clock 不经济成立） | 低 |
| 分辨率减负 | P2 低优挂账（考跑 40min/100 天已可接受，收益小） | 低 |
| EventBus 全量计数口 | **维持 HasSubscribers 替代方案**挂账观察（YAGNI 先例 HH.63；全量口等真实诊断需求再上） | 低 |

### 9.4 教训库沉淀裁定（钩子3）

- **L-11 入库**：跨轮残留类漏项单轮绿不算绿，建拆建第 3 轮才是试金石（v2 R1/R2 全绿假象掩盖 DamageSystem 漏项实锤；防范动作=生命周期类验收硬条款「≥3 轮+以第 3 轮断言为判定口径」+容器汇总行禁作验收口径）。
- **L-12 入库**：同文件多处 SearchReplace 禁止并行（后者覆盖前者）——HH.42 家族第 10 笔新变体，触发红线复盘：L-02 已入库但防范动作未覆盖「同文件多编辑并行」子形态→防范动作升级为铁律（同文件多处编辑单线程串行+每处编辑后即验证）。
- 教训核查：除上述两条外无新增。

### 9.5 销号与账务

- 账本：HH.92 → ✅验收成立销号（D552）；HH.93 → ✅验收成立销号（D552）。
- T13 收尾三修（DamageSystem.ResetState+ExecuteAttack 防御+HappinessSystem 建键）追认入库——两处防御只在原本必炸路径生效、玩家侧开局当日死亡同源受益，§七 红线自查采信。
- 队列：测试环境批行 → ✅；P0 调优批行注入 §9.3 归口（件F）；挂账池新增条件触发行（1x 对照/分辨率/EventBus 口）。
- 验收三问（钩子2）结论：①缺陷根因=A2 为 AI 评分域既有缺口（非本批引入）/容器判定缺陷=新工装首版汇总逻辑单查 results/三修漏项=跨轮残留单轮不可见；②定性=后两者属流程漏洞非执行端单次失误（执行端自驱三修+如实披露已尽责）；③库状态=L-11 缺条目新增、L-12=有条（L-02）没查的子形态缺口→红线升级（见教训库红线复盘区）。
