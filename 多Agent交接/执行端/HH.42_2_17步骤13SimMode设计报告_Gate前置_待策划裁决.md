...
- ⚠️ **勘误（2026-08-31，正文更正）**：本报告起草时记“索引未登记 HH.41（停在 HH.40+重复行）”系读到旧缓存所致**误报**——实核当前 _交接索引.md：HH.41 已由策划端登记（043455c）、HH.40 验收已回填 ✅，HH.40/HH.41 各居一行无重复；仅 _任务队列.md 的 Q9→✅ 与 13 解除阻塞仍未反映（队列只读，执行端不改，报策划端回写）。
...
## 策划裁决（2026-08-31 策划端回写；用户四点拍板，全裁 A）

> 策划端实盘复核：两 commit 构成属实（97f47c6/5176430）；关键代码声明抽查 9 项全绿；Q9 两探针证据自洽成立（挂账销行）；执行端「队列仍未反映 Q9→✅」经实核为旧缓存误报（043455c 已回写，队列无需补写）。

| 决策点 | 裁决 | 理由 |
|--------|------|------|
| 1 Satiety 归属（13/14） | **A 归步骤14**（D453） | 进食=经济闭环内容，与 AbstractEconomySettler 同域；Fine 实体扣国库+Abstract 计数进食同语义一次对齐，拆两处要改两遍；步骤13 前现状延续无回归 |
| 2 Siege 生产链（能力打通 vs 接触发） | **A 能力打通+触发延后军事期**（D454） | 消除 false 占位半退役态=真清偿；触发方（UtilityAction/SiegeWorkshop 前置）属军事期内容，现在接了无消费方——随 2_18 议（同「AI 城堡升级」先例） |
| 3 MidChunkLodState 回收口径 | **A 整体清空即回收**（D455） | 逐条回收时间相关，威胁同 seed 逐字节确定性红线+热度记忆丢失影响战斗热点传播；稀疏登记有界，内存可忽略 |
| 4 存档记录 SimMode | **A 不入档**（D456） | simMode=每日重判派生视图案（D347 五步①）；D334 实体常驻读档默认 Fine 无跳变；零存档 schema 风险（kingdoms[] 迁移归 2_11） |

### 分歧裁决记录
- 无分歧：执行端推荐与策划端独立评估四点全部一致，均裁 A。

### 衍生产物
- 设计文档修订：✅ 2_17 三处已由策划端按 D456 修订（§1.1 接缝表 2_11 存档行去「SimMode 入档」/§3.3 判定块「存档记录 SimMode」句改不入档/D333 行同步）；0.6 §四十三 登记 D453~D456。
- 新建清单任务：执行端按 §3.2 批A（判定地基）→ 批B（休眠唤醒）→ 批C（债清偿+Smoke_13）实施；队列 13 行转「Gate 已裁，实施中」；挂账池四行（Q9 探针销/Siege 拆分/Satiety 改挂14/缺口径① 定案）已同步。
- sim-sync 义务判定（策划端）：步骤13 三批不涉 AI.Core 决策核公式/champion 参数 → 无 T 级义务；SimModeConfig 参数 sim 侧无消费面 → 不登记 15_差距账本。
- 附带纠偏：本报告 §五.3「补登索引 HH.41」建议系旧缓存误报残留（实核索引 HH.40/41/42 各居一行），无需执行；§五.4 判定采纳（无 sim-sync 义务）。
