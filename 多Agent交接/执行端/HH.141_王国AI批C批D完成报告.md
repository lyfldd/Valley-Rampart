...
## 九、策划裁决区（D598，2026-09-10 已回写）

**验收成立销号**（HH.140/141，commit 4297f33+a27df02）：七点实盘复核全过=①diff-tree 构成吻合+AI.Core 文件级零命中②玩家侧零改动（FormationController 纯新增 override 实读）③D594 整改令两条款兑现（IsPrefabMissing 同源门控+P6b/P6c 双向探针）④批C 逐项在场⑤批D 逐项在场+15_账本一·补十九⑥探针 23/23 实读+六容器收口⑦同 seed 确定性（P7c）。

**列报 5 项裁决**：

| # | 列报 | 裁决 |
|---|------|------|
| 1 | 批B 遗留缺口修复申报 | **裁=语义补全非放宽**（让批B 交付真实生效；建造三 case 三守卫全继承+TrainGeneral 镜像直读采信）；实盘直读自纠嘉奖；「评分侧可达性」断言意识入 L-21 防范域 |
| 2 | 🔴守军编队跨国招兵 | **玩法破坏级成立；裁方案 A**=FindIdleSoldiers 增 kingdomId 过滤（过滤面从 faction 共享 camp 粒度迁 kingdomId 归属真源；ResolveKingdomId 派生+<0 fallback 旧行为；B7/玩家编队/补员链同函数全受益）——**另立微批 HH.144**（与列报3 同串，批E 前置）；任务书=多Agent交接/策划端/HH.144 |
| 3 | F1 带外 clamp 口径 | **裁改**：分母=max(主城-威胁锚实际 Chebyshev 距离, 带径 maxR×div)，威胁≈0 置 0 不变。依据=2_22 §3.7 验收判据是「朝向性」相对判据（距主威胁边境<中位数），clamp 恒 0 使其在大半地图形态下系统性不可达=w1 空转；批E 前改零训练成本（E3 未动）；P7a/P7c 断言随改同步重跑 |
| 4 | KingdomId override 注记 | **追认**：向后兼容实读（<0=旧链原样）+守军计数面正确性根基；A6 归属推导链语义升级=override→将军→首成员→-1（注记入档）；列报2 落地后 override=编队归属唯一可靠面，两改动互相咬合 |
| 5 | D5 特征语义确认 | **确认符合设计意图**：四象限种族自洽（人类临河/精灵密林/矮人矿区 Mine≥0.05/兽人开阔 Plain≥0.60）+阈值全 SO 化；口径钉死=BarrenRich=可建开阔地密度（非字面贫瘠）；D316 原设计仅枚举名，执行端推导合法 |

**P5 三重修复裁定**：faction 镜像 B7 先例=实现完整性（对齐场景单位既有数据面）；SetKingdomIdOverride=最小语义扩展（追认）；空壳即毁=防御性兜底正面样本（与批B D594 同型缺陷，触发红线复盘→**L-21 入库**）。

**教训核查**：L-21 新入库（目标驱动循环计数面可达性，B8 黑洞→P5 空壳二次实证=红线复盘首条）+L-13 增补（refresh_unity 假成功变体）+L-16 增补（Play 态重入全清）。

**注记**：C6 被攻分支无独立行为探针（P3 只覆盖军覆；消费路径同源由 P3 承载，批E E4 冒烟覆盖）；整改令可选条款（执行侧防御预检）未落=合规，2_18 事件驱动接入时须补。

**批E 解锁附前置**=HH.144 微批完成（E4 冒烟九项按修复后口径跑防返工）。详见 0.6 §一百二十七。
