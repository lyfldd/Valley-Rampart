...
## 九、策划端裁决区（D608，2026-09-10：验收成立销号）

> **验收成立销号**——策划端实盘复核全实锤零失实，2_22 P0 批A~E 全数收口（D608 落 0.6 §一百三十七）。

**实盘复核实录（策划端逐笔验证）**：
1. **commit 链核验**：主仓 a5bd58b（E1：7 文件 288+，KingdomBrainSave.cs 190 行新档+meta）/03f9703（E2：SnapshotAggregator 49 行+meta）/474488a（E4：Smoke_2_22P0.cs 790 行+KingdomBrain.cs 5 行接线）/f196cbc（报告）/9e4a5d8（git-plan-sync）构成与申报一致；**AI.Core diff-tree 空集=红线零直改亲验**；训练仓 9fbb237c=6 文件 454+（harness/KingdomBrain 三件镜像 346 行+registry_overrides.json+账本）。
2. **E4 探针双跑亲验**：`Logs/P1/smoke_2_22p0_run{1,2}.log` 直读——run1/run2 各 32 条 PASS（31 探针行+1 正门行）FAIL=0；排除 tag 标识归一化比对 **diff=0 逐字节一致成立**；P7 三件套强验证实录在档（权重 1.7==1.7/姿态 Alert==Alert/流水 1==1，非 0==0 弱验证）；L-21 双断言在场（P3a 选招落地 2 次+P8d 失败终止）。
3. **MachineCount 接线直读**：KingdomBrain.cs L339-340 `SiegeProductionSystem.Instance.GetPlacedMachineCountByKingdom(kingdomId)`+null 守卫，5 行 diff 与申报一致；P9f 实录 `快照 MachineCount=2 落位=2` 在档。
4. **E2 MD5 亲验**：策划端本端重算双端 `BattleLearnedWeights.cs` MD5=`0C1971A65E0E532280AAA88251967E96` 相等（与报告申报逐字一致）。
5. **E1 存档纪律**：KingdomBrainSave `SaveId="KingdomBrainAI"`+模块自治 `version=1`，全局 saveVersion 零 bump 符合 2_11 纯增量纪律。
6. **容器正门**：Smoke_2_22P0.cs 直读=EnterTestRun 正门+ExitTestRun 收尾+SEED=21140，D600 勘正兑现。

**列报 5 项裁决**（HH.149 §八）：
| # | 列报 | 裁决 |
|---|------|------|
| 1 | 确定性双跑纪律（run2 必须退 Play 重进，ResetWorldForNext 清场不彻底 P1e 5 vs 11 实证） | **采纳固化**：批注 L-22 入教训库（确定性双跑=干净局对称纪律）；17 手册双跑纪律条款归执行端下串顺手补一行（不另立批） |
| 2 | 2_20B/2_20C 裸局回归受限（批E 代码零改动） | **采信（环境限制非批E 缺陷）**：迁移验证归 HH.150，其交付须含 2_20B/2_20C 迁移后补跑+批E 触碰面回归说明 |
| 3 | MachineCount A2 接线修复（产品级，P9f 实锤） | **追认嘉奖**：冒烟容器首跑实锤产品缺口+单源计数修复=D570 口径闭环，属冒烟体系正向产出（与 L-21 防线设计同向） |
| 4 | P3a 门控注入语义（D348 已达目标不选招=产品正确） | **认可**：探针造「未达目标」前提+用毕复原=探针级临时态合规，非产品逻辑改动 |
| 5 | B8 机器 prefab 依赖（Ballista 内存借用不入盘） | **知悉**：与 D582② 扣费无生成语义一致；prefab 依赖维持挂美术批7 |

**验收三问（钩子2）**：①申报失实=零（双跑日志/MD5/接线 diff 全亲验命中）；②流程漏洞=确定性双跑的「干净局对称」此前无成文纪律（执行端五轮自纠内消化，非缺陷泄漏）→L-22 当场入库；③教训核查=L-22 新增（钩子3 首例随验收入册）。

**嘉奖**：①五轮自纠链全程留痕且把 P9f 探针失败转化为产品级修复（P0 冒烟体系设计意图的完整兑现）②P7 强验证设计（注入→存→破坏→读→复原 1==1，杜绝平凡通过）③F7 规范格式规避 .NET R 格式双端差异（对拍工程细节扎实）④双跑比对脚手架+诚实列报 2_20B/2_20C 受限不掩饰。

**王国AI P0（2_22 批A~E）全批收官**。下一步：执行端=HH.150 测试基建微批（存量 9 处裸局迁移+2_20B/2_20C 迁移后补跑）；策划端=2_23 资源 P0 批A 解锁跟踪（R-A1 依赖军事批A 已达成）+P1 七考挂待观察。
