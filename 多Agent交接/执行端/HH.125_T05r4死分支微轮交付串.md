...
## §策划裁决区（策划端回写 · D586 · 2026-09-09 验收成立销号=结构面终收口）

**实盘复核全过**（策划端本端直验）：①训练仓 22a6f17=3 files（D-021 §6.5 +21 行/T05 README 收口态/SimFormation.cs 16 行）/主仓 437d236=3 files 构成吻合；②r4 verdict 直读=rejected **7 退化点**（W9_T02_2 -5.0%/KT04_2_0 -8.5%/KT09_2_0/KT10_2_1 -26.3%/KT11_2_0/KT15_2_0 -50.7%/KT15_2_2 -100%）与报告逐条吻合；r5 verdict=**candidate delta=0.002 regression 空** holdout 0.506 δ0.000；③**r5 行为痕迹与恢复实锤**（name 配对）：KT05_2_0 kd 4.63（③点火 Charge 痕迹）/KT15_2_0 sub 0.276+sup 123 全等 base/KT04_2_0 sub 0.448+kd 2.761+sup 4 全等 base/W9_T18_2 sub 0.027+slot 0.554+kd 0.497 全等（**D584① 成果无回退**）/r5 vs base |Δ|>0.002 **零场景**；④代码 diff 实锤：同分裁决序对齐（倒序链+`>=`：Sally→Charge→Retreat→Defense，注释「不同分仍是谁大谁赢=①④竞争等既有行为不变」）+advance 语义对齐（`Sally || (Charge && heat < fbHeatEngage)`——②域才设推进目标，③域纯冲锋+fbSupportSearchRadius=150 全图热点误覆盖根因注记）；⑤**前置对照第一问兑现**：表路径 rule③=return 序无竞争活路径（garrison/attack 可达）+KT05_2_0 garrison trace 实证（表 Charge 持续全场 vs 加权 Defense）=**仅加权死，分歧面收敛**；死分支前提入档 D-021 §6.5（fbHeatCharge 0.6>fbHeatEngage 0.3 → ③域⊂④域+权重同源 → 结构性同分；参数面重排时前提瓦解注记）。**两项裁决**：
1. **r5 形态=结构面终收口：准**——D584② 验收两条全过（①全域回归门过：regression 空+全域 vs base 仅 KT05_2_2 +0.002/KT17_2_2 -0.003 噪声带内 ②③点火路径行为出现：KT05_2_0 kd 痕迹+双路径 trace）+W9_T18_2 维持全等+r4 七退化点全消+两步修复均表路径语义对齐无参数找补（switchDebounce 及全部 fb* 键值未动实锤）。**T05 结构面收口**（fb 连续化落地：行为等价+路径连续可训练+③点火活）。
2. **参数面开工=准（附评分面前置）**：死分支微轮完成（fbHeatCharge 死键→活键=罪1 前置清偿）——**评分面设计稿=策划端本轮后立即起草、训练师收到即可开工 fb* 键族快验制双探针**（D581④/D584③ 承诺兑现窗口）。

**嘉奖**：r4 rejected 不硬闯=两层归因纪律（裁决序对齐→advance 误设，修复 A 维度暴露 B 维度分歧=「结构对齐全维度」案例，17 手册候选）；前置对照第一问先行（D584② 责令逐条兑现）；并发写纪律执行记录（HH.124 行误替换当即补回+grep 双行验讫+D585 并发写入确认）。HH.125 销号。
