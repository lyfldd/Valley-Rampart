...
- 阈值表 + 最小停留 **全部落 `KingdomBrainConfig` SO**（`Data/KingdomBrainConfig.cs` + asset `Resources/Config/Kingdoms/KingBrainsnBrainConfig.asset`，so-data-driven 无魔法数；对齐实施计划 §三）：
