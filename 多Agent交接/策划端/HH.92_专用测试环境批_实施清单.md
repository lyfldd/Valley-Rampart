...
## §六 排期（🔴全局窗口修正版：用户澄清本批仅占 3 个施工日，游戏全部内容须在全局窗口内收口）

| 施工日 | 内容 | 里程碑 |
|--------|------|--------|
| 前置半日 | **HH.89 度量衡批收口**（=P0-1） | TimeManager 360 基线就绪 |
| 第1日 | P0-2/P0-3(T0)+T5+T1+T2+T3 | 时序根治+时间层完成 |
| 第2日 | T4+T6+T7+T8+T9 | 三档对照+fixture 骨架 |
| 第3日 | T10~T12+T13+T15（并行）→T16/T17 抽查口径→T18/T19+报告 HH.93 | **夹具层全通+MVP 验收** |

- **压缩口径**（第3日）：T14 核对册=运行时单例高优核全、Rendering/其余列报；T16=行动池 21 条抽查冒烟（每条触发证据在案即可，核对表边跑边补）；T17=逻辑入口核/UI 入口列报。
- **全局链**（本批之后）：P0 调优批+P1 六考（2 日）→王国AI P0 交错地图（双清单 **MVP 化压缩待用户拍板**：建议批A~C 核心+批D/E 缩减）→全局收口缓冲。美术轨（精灵主城 7 张+验证图+矮人居民×1）=用户手作并行。
- 假红回退窗口并入各日（T4 三档对照若 15x 假红即回退 10x 列报，不单独占日）。
