...
//    ~~缺值挂账（HH.54 §三.2，2026-09-03）~~ → **已真值回填收口**（HH.115 E-T7 抄表核实时资产已全量真值化，
//    原两条散点锚已并入总表；消费侧缺表回退=1.0 中性显式常量，见 KingdomRace.GetGatherMul/TrainingSystem）。
//
//  **乘数真值总表（12 项×4 族，HH.115 E-T7 固化 2026-09-08，抄自 Resources/Config/Races/ 四资产逐行）**
//  | 乘数           | Human | Elf  | Dwarf | Orc  | 备注（权威=2_20.1 §二）                    |
//  |----------------|-------|------|-------|------|--------------------------------------------|
//  | trainCostMul   | 1.0   | 1.05 | 1.10  | 0.85 | 军训成本%（散点锚：兽人 0.85）             |
//  | trainSpeedMul  | 1.0   | 1.0  | 0.95  | 0.85 | 训练时长%（<1 加速）                       |
//  | meleeAtkMul    | 1.0   | 0.9  | 1.15  | 1.15 | 近战攻%                                    |
//  | rangedAtkMul   | 1.0   | 1.2  | 0.8   | 0.85 | 远程攻%（近战远程分离纪律）                |
//  | moveSpeedMul   | 1.0   | 1.15 | 0.85  | 1.05 | 移速%                                      |
//  | mineMul        | 1.0   | 0.9  | 1.30  | 1.05 | 采矿%（散点锚：矮人 1.30）                 |
//  | lumberMul      | 1.0   | 1.25 | 0.9   | 0.85 | 伐木%                                      |
//  | farmMul        | 1.0   | 1.1  | 0.75  | 0.7  | 粮产%                                      |
//  | buildSpeedMul  | 1.0   | 0.9  | 1.2   | 0.8  | 建造速度%                                  |
//  | buildingHpMul  | 1.0   | 0.9  | 1.25  | 0.85 | 建筑血量%                                  |
//  | carryCapMul    | 1.0   | 0.95 | 1.2   | 0.9  | 携带上限%                                  |
//  | 缺表回退       | 1.0   | 1.0  | 1.0   | 1.0  | RaceDef 查表失败=1.0 中性（显式常量）      |
//  三方核对（E-T7）：sim 侧 RaceDef 乘数无对应 champion/factor_registry 参数（训练侧 professions/tuning
//  无 race 经济乘数域）→ 零回灌义务，见 15_训练侧差距文档 HH.115 注记行。
//
