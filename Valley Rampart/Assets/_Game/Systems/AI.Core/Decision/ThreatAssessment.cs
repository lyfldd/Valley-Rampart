// ============================================================================
//  AI.Core Decision - 威胁评定（从壳 ThreatAssessment.cs 迁入）
//  03_大脑提取与双适配工程.md 迁移 6 步·步3。
//  接缝替换（接缝 4）：AttentionTuningConfig -> TuningSnapshot；
...
public class ThreatAssessor
{
    ...
    public ThreatAssessmentResult Update(
        ...
        float hpRatio, bool isNight, TuningSnapshot config,
        ProfessionSnapshot profession, float currentTime)
    {
    ...
    public static float CalculateRawFactor(
        ...
        int allyCount,
        bool isNight,
        ProfessionSnapshot profession,
        TuningSnapshot config,
        ...
        float regionHeat)
    {
        ...
        if (enemyCount > 0 && nearestEnemyDist < perceptionWorldRadius)
        {
            if (attackWorldRange > 0f && nearestEnemyDist <= attackWorldRange)
                ...
                distFactor = 1f - MathfX.Clamp01(nearestEnemyDist / perceptionWorldRadius);
        }
        else
        {
            distFactor = 0f;
        ...
        float countFactor = MathfX.Clamp01(enemyCount / config.countFactorFullCount);

        // 血量因子（越低越高，0-1）
        float hpFactor = 1f - MathfX.Clamp01(hpRatio);

        // 友军保护因子（越多友军越低，0-1）
        float allyFactor = 1f - MathfX.Clamp01((float)allyCount / config.protectionFriendThreshold);

        // 时间因子（夜晚 +0.1）
        float timeFactor = isNight ? 0.1f : 0f;

        // 3.0.1_LOD §3.2 区块威胁热度因子（环境型威胁，与夜晚同构——无 ThreatStimulus 也能推高威胁）
        float heatFactor = MathfX.Clamp01(regionHeat);
        ...
        if (attackWorldRange > 0f && nearestEnemyDist <= attackWorldRange)
            x = MathfX.Max(x, config.closeRangeMinRaw);
