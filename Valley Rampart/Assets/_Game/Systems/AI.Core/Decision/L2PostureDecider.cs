// ============================================================================
//  AI.Core Decision - L2 姿态裁决层（从壳 Decision/L2PostureDecider.cs 迁入）
//  03_大脑提取与双适配工程.md 迁移 6 步·步3。
//  接缝替换：
//    接缝 1：IDamageable -> IUnitHandle（战术短撤敌人引用）；
//    接缝 2：IsDestroyed(UnityEngine.Object 伪 null) -> IUnitHandle.IsAlive（接口自带伪 null 检测）；
...
public static class L2PostureDecider
{
    ...
    private static BehaviorSpectrum CalculateSpectrum(in FactorContext ctx)
    {
        ...
        threshold = MathfX.Max(config.thresholdMinFloor, threshold / config.thresholdScaleDivisor);  // 连续量纲，保底防除零
        ...
        float resistTotal = MathfX.Min(config.l2ResistCap, formationResist * config.l2FormationResistScale + workResist); // 抗性封顶防负
    ...
    {
        int maxHitCount = ctx.Profession.maxHitCount;
        ...
        IUnitHandle enemy = FindActiveThreatEnemy(in ctx);
        if (enemy != null && enemy.IsAlive)  // IsAlive 含伪 null 检测（接缝 2）
        {
            posture.IsTacticalRetreat = true;
            // 战术方向来源：活跃 ThreatStimulus.enemy 位置反方向（单位向量）
            posture.MoveTarget = (ctx.SelfPos - enemy.Position).normalized;
    ...
    /// </summary>
    private static IUnitHandle FindActiveThreatEnemy(in FactorContext ctx)
    {
        // L1 焦点是威胁层且有 Enemy 引用（ts.Enemy 的 !=null 同样不触发销毁检测，需 IsAlive 防御）
