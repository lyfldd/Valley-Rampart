// ============================================================================
//  AI.Core Decision - 三层裁决管线数据结构（从壳 Decision/DecisionStructs.cs 迁入）
//  03_大脑提取与双适配工程.md 迁移 6 步·步3。
//  L1 输出 FocusDecision -> L2 输出 PostureDecision -> L3 输出 BehaviorCommand
//  全部 struct 零 GC，符合纯计算管线契约
//  接缝替换：Vector2 -> Vector2X；Vector2Int -> Vector2IntX；
//           BehaviorCommand.Anchor: UnitController -> IUnitHandle（接缝 1）；
//           PostureDecision.TacticalRetreatEnemy: IDamageable -> IUnitHandle（接缝 1）。
// ============================================================================
...
public struct PostureDecision
{
    ...
    public IUnitHandle TacticalRetreatEnemy;
}
...
public struct BehaviorCommand
{
    ...
    /// <summary>仅 FollowAnchor 用（IUnitHandle 抽象，接缝 1：原 UnitController）</summary>
    public IUnitHandle Anchor;
    public float Speed;
