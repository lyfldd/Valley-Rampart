// ============================================================================
//  AI.Core Stimulus - 新增刺激源（从壳 NewStimuli.cs 迁入）
//  03_大脑提取与双适配工程.md 迁移 6 步·步3：Stimulus structs/classes 搬入核。
//  全部 class 池化，防 struct 装箱（§9 零 GC 纪律）。
//  接缝替换：FollowStimulus.Anchor: UnitController -> IUnitHandle（接缝 1）；
//           Vector2 -> Vector2X；Vector2Int -> Vector2IntX。
// ============================================================================
...
/// 非编队跟随（工人随军）SlotOffset = zero，退化为原松散跟随语义。
/// </summary>
public sealed class FollowStimulus : IStimulus
{
    public AttentionLayer Layer => AttentionLayer.Task;
    /// <summary>锚点（部队队长/将军），位置每 tick 随锚点刷新（IUnitHandle 抽象，接缝 1）</summary>
    ...
    public Vector2IntX SlotOffset = Vector2IntX.zero;
    ...
    public Vector2X Position => Anchor != null ? Anchor.Position : Vector2X.zero;
    public float Intensity { get; set; }
