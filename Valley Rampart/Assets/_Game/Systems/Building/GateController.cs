...
/// <summary>
/// 城门状态机（3.5.4 §8.4；P1-13）。挂在城门单位（Occupation.Gate，挂 FortificationDef）上。
...
///
/// 实现要点：FortificationDef 是共享 SO（改它污染所有城门），故本组件把可通行状态写入
/// UnitController.FortificationPassableOverride（运行时覆盖），不污染共享资产。
...
public class GateController : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        ApplyToUnit();   // 初始按当前时段应用（OnEnable 时 CurrentPhase 已就绪）
    }
    ...
    /// <summary>把当前 GateState 写入 UnitController.FortificationPassableOverride（Open=可通行）。</summary>
